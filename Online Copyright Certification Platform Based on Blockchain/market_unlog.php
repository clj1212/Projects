		...
		<div id="sticky_navigation">
			<div class="demo_container navigation-bar">
				<div class="navigation">
					<div class="logo"><a href="index.html">主页</a></div>
					...
					<div class="navig">
						<ul>
							<li>交易商城</li>
							<li><a href="ourinfo.html">关于我们</a></li>
						</ul>
					</div>
				...
				<div class="navigation-right">
					<ul class="user">
						<li>
							<span></span><a href="login.html">登录</a>
						</li>
						<li>
...
<section id="main">
	<div class="content">
		...
		<div class="coats">
			...
			<small><a href="login.html">登录发现更多...</a></small>
			<div class="coat-row">
				<?php
					...
					session_start();
						...
						$sql="select * from tradable where s1=1";
...
?>
		...
		<div class="coats">
			...
			<div class="coat-row">
				<?php
						mysqli_set_charset( $conn,"utf8" );//使数据库中的数据以utf8格式显示在网页上
						$sql="select * from tradable where s2=1";
						$result=mysqli_query($conn,$sql); 
						if (!$result) {
							printf("Error: %s\n", mysqli_error($conn));
							exit();}
...
<small><a href="login.html">登录发现更多...</a></small>
			<div class="coat-row">
				<?php
						mysqli_set_charset( $conn,"utf8" );//使数据库中的数据以utf8格式显示在网页上
						$sql="select * from tradable where s3=1";
						$result=mysqli_query($conn,$sql); 
						if (!$result) {
							printf("Error: %s\n", mysqli_error($conn));
							exit();}
...
<small><a href="login.html">登录发现更多...</a></small>
			<div class="coat-row">
				<?php
						mysqli_set_charset( $conn,"utf8" );//使数据库中的数据以utf8格式显示在网页上
						$sql="select * from tradable where s4=1";
						$result=mysqli_query($conn,$sql); 
						if (!$result) {
							printf("Error: %s\n", mysqli_error($conn));
							exit();}
...
<small><a href="login.html">登录发现更多...</a></small>
			<div class="coat-row">
				<?php
						mysqli_set_charset( $conn,"utf8" );//使数据库中的数据以utf8格式显示在网页上
						$sql="select * from tradable where s5=1";
						$result=mysqli_query($conn,$sql); 
						if (!$result) {
							printf("Error: %s\n", mysqli_error($conn));
							exit();}
